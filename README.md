...
竞彩足球概率分析与研究项目。当前已提供 Football-Data 历史比赛与市场数据导入管道；不训练预测模型、不承诺收益，也不自动购票。
...
- 支持目录中配置的 38 个公开联赛；默认范围是每个联赛最近 5 个**已完成**赛季。
...
- `POST /api/data/import`：触发导入。空对象使用默认的 38 个联赛与最近 5 个已完成赛季；显式范围只接受目录内联赛代码与已完成赛季，最多 5 个赛季。
