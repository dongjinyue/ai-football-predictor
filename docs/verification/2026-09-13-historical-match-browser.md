...
打开 `http://127.0.0.1:4173/#history`。浏览页不会触发导入；数据库为空时会显示“尚未导入历史比赛”，需先按 README 的导入说明写入真实数据。
...
本机 Edge（浏览器）以 `http://127.0.0.1:4173/#history` 打开真实服务后，读取到：
