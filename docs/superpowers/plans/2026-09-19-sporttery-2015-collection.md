...
- [x] 写失败测试，模拟两页列表、重复比赛、中断恢复、奖金缺失和 567 停止。
- [x] 实现 7 天窗口、动态页数、去重队列、每项完成后检查点写入和结构化进度日志。
- [x] 实现 `--delay-min`、`--delay-max`、`--resume` 和 `--dry-run` 参数，并校验日期与延迟范围。
- [x] 运行服务测试与全部后端回归测试。
...
- [x] 先运行 `collect --start 2015-01-01 --end 2015-01-03 --delay-min 3 --delay-max 5`，核对列表 44 场、2 页及已知比赛 `matchId=62373`。
- [x] 对小窗口重复执行一次，确认比赛和快照计数不增长且原始文件命中缓存。
- [ ] 运行全年命令，保留实时检查点；若遇到 567，停止并报告实际完成范围，不规避来源限制。
- [ ] 运行 `report --year 2015`，输出唯一比赛、五类玩法覆盖、快照分布、无效记录和原始文件校验结果。
- [x] 更新 README 的运行命令、预计时长、恢复方式和数据不进入 Git 的说明。
